<?php
/**
 * Gestion des Médecins - Administrateur
 */
...
try {
    ...
    // Actions POST (suspension, activation, suppression)
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $action = $_POST['action'] ?? '';
        $uid = (int)($_POST['utilisateur_id'] ?? 0);

        if ($action === 'suspendre' && $uid) {
            $db->prepare("UPDATE utilisateurs SET statut='suspendu' WHERE id=? AND role='medecin'")->execute([$uid]);
            $message = "Médecin suspendu."; $message_type = 'warning';
        } elseif ($action === 'activer' && $uid) {
            $db->prepare("UPDATE utilisateurs SET statut='actif' WHERE id=? AND role='medecin'")->execute([$uid]);
            $message = "Médecin activé."; $message_type = 'success';
        } elseif ($action === 'supprimer' && $uid) {
            $db->prepare("DELETE FROM utilisateurs WHERE id=? AND role='medecin'")->execute([$uid]);
            $message = "Médecin supprimé."; $message_type = 'danger';
        } elseif ($action === 'toggle_dispo') {
            $mid = (int)($_POST['medecin_id'] ?? 0);
            $dispo = (int)($_POST['disponible'] ?? 0);
            $db->prepare("UPDATE medecins SET disponible=? WHERE id=?")->execute([!$dispo, $mid]);
            $message = "Disponibilité mise à jour."; $message_type = 'success';
        }
    }
    ...
    $where = "WHERE u.role='medecin'";
    $params = [];
    if ($filtre === 'actif')    { $where .= " AND u.statut='actif'"; }
    if ($filtre === 'suspendu') { $where .= " AND u.statut='suspendu'"; }
    if ($filtre === 'dispo')    { $where .= " AND m.disponible=1"; }
    if ($search) {
        $where .= " AND (u.nom LIKE ? OR u.prenom LIKE ? OR u.email LIKE ? OR m.specialite LIKE ?)";
        $s = "%$search%";
        $params = [$s,$s,$s,$s];
    }
    ...
    $stmt = $db->prepare("
        SELECT u.id as utilisateur_id, u.nom, u.prenom, u.email, u.telephone,
               u.statut, u.date_inscription, u.ville,
               m.id as medecin_id, m.specialite, m.numero_medecin, m.annees_experience,
               m.tarif_consultation, m.note_moyenne, m.nombre_consultations, m.disponible
        FROM utilisateurs u
        INNER JOIN medecins m ON m.utilisateur_id = u.id
        $where
    ...
    // Compteurs
    $c_total    = $db->query("SELECT COUNT(*) FROM medecins")->fetchColumn();
    $c_actif    = $db->query("SELECT COUNT(*) FROM utilisateurs u INNER JOIN medecins m ON m.utilisateur_id=u.id WHERE u.statut='actif'")->fetchColumn();
    $c_suspendu = $db->query("SELECT COUNT(*) FROM utilisateurs u INNER JOIN medecins m ON m.utilisateur_id=u.id WHERE u.statut='suspendu'")->fetchColumn();
    $c_dispo    = $db->query("SELECT COUNT(*) FROM medecins WHERE disponible=1")->fetchColumn();

} catch(PDOException $e) { error_log($e->getMessage()); $medecins = []; }
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
    <title>Médecins - Admin - <?php echo SITE_NAME; ?></title>
    ...
    <style>
        ...
        *{box-sizing:border-box;margin:0;padding:0}body{font-family:'Outfit',sans-serif;background:#f0f2f8;color:#1a1a2e}
        ...
        td{padding:14px 20px;font-size:13px;border-bottom:1px solid #f0f2f5;vertical-align:middle}
        ...
        .btn-xs:hover{opacity:.8;transform:translateY(-1px)}
        .dispo-toggle{width:42px;height:22px;border-radius:20px;border:none;cursor:pointer;position:relative;transition:all .3s;font-size:0}
        .dispo-on{background:#22c55e}.dispo-off{background:#d1d5db}
        .dispo-toggle::after{content:'';width:18px;height:18px;border-radius:50%;background:#fff;position:absolute;top:2px;transition:all .3s;box-shadow:0 1px 4px rgba(0,0,0,.2)}
        .dispo-on::after{left:22px}.dispo-off::after{left:2px}
        ...
        .modal{background:#fff;border-radius:20px;padding:32px;max-width:420px;width:90%;box-shadow:0 20px 60px rgba(0,0,0,.2)}
        .modal h4{font-size:18px;font-weight:700;margin-bottom:8px}
...
<aside class="sidebar">
    ...
    <nav class="sidebar-menu">
        ...
        <a href="medecins.php" class="active"><i class="bi bi-person-badge"></i> Médecins</a>
...
<div class="main-content">
    <div class="topbar">
        <div><h2>Gestion des Médecins</h2><p><?php echo count($medecins); ?> médecin(s) trouvé(s)</p></div>
        <a href="utilisateurs.php?action=ajouter&role=medecin" class="btn btn-primary"><i class="bi bi-person-plus"></i> Ajouter un médecin</a>
    </div>
    <div class="content">

        <?php if ($message): ?>
        <div class="alert alert-<?php echo $message_type; ?>"><i class="bi bi-check-circle"></i><?php echo $message; ?></div>
        <?php endif; ?>

        <!-- Mini stats -->
        <div class="mini-stats">
            <a href="?filtre=tous" class="mini-card <?php echo $filtre==='tous'?'active':''; ?>">
                <div class="n"><?php echo $c_total; ?></div><p>Total</p>
            </a>
            ...
            <a href="?filtre=dispo" class="mini-card <?php echo $filtre==='dispo'?'active':''; ?>">
                <div class="n" style="color:#667eea"><?php echo $c_dispo; ?></div><p>Disponibles</p>
            </a>
        </div>
        ...
        <div class="toolbar">
            <form method="GET" style="display:flex;gap:12px;flex:1;flex-wrap:wrap">
                ...
                <div class="search-box">
                    ...
                    <input type="text" name="q" placeholder="Rechercher un médecin..." value="<?php echo htmlspecialchars($search); ?>">
                </div>
                <button type="submit" class="btn btn-primary"><i class="bi bi-search"></i> Rechercher</button>
                <?php if ($search): ?><a href="medecins.php" class="btn btn-outline"><i class="bi bi-x"></i> Effacer</a><?php endif; ?>
            </form>
        </div>
        ...
        <div class="card">
            <div class="card-header">
                <h5><i class="bi bi-person-badge" style="color:var(--primary)"></i> Liste des médecins</h5>
            </div>
                ...
                <tr>
                    <td>
                        <div class="user-cell">
                            ...
                            <div class="uinfo">
                                <h6>Dr. <?php echo htmlspecialchars($m['prenom'].' '.$m['nom']); ?></h6>
                                <p><?php echo htmlspecialchars($m['email']); ?></p>
                                <p><?php echo htmlspecialchars($m['numero_medecin']); ?></p>
                            </div>
                        </div>
                    </td>
                    <td><?php echo htmlspecialchars($m['specialite']); ?></td>
                    <td><?php echo number_format($m['tarif_consultation'],0,',',' '); ?> FCFA</td>
                    <td style="text-align:center;font-weight:700;color:var(--primary)"><?php echo $m['nombre_consultations']; ?></td>
                    <td>
                        <div class="stars">
                            <?php for ($s=1;$s<=5;$s++) echo $s<=$m['note_moyenne']?'★':'☆'; ?>
                        </div>
                        <span style="font-size:11px;color:#888"><?php echo number_format($m['note_moyenne'],1); ?>/5</span>
                    </td>
                    <td>
                        <form method="POST" style="display:inline">
                            <input type="hidden" name="action" value="toggle_dispo">
                            <input type="hidden" name="medecin_id" value="<?php echo $m['medecin_id']; ?>">
                            <input type="hidden" name="disponible" value="<?php echo $m['disponible']; ?>">
                            <button type="submit" class="dispo-toggle <?php echo $m['disponible']?'dispo-on':'dispo-off'; ?>" title="<?php echo $m['disponible']?'Disponible — cliquer pour désactiver':'Indisponible — cliquer pour activer'; ?>"></button>
                        </form>
                    </td>
                    <td>
                        <span class="badge <?php echo $m['statut']==='actif'?'badge-success':($m['statut']==='suspendu'?'badge-danger':'badge-warning'); ?>">
                            <?php echo ucfirst($m['statut']); ?>
                        </span>
                    </td>
                    <td>
                        <div class="actions">
                            <a href="medecin-detail.php?id=<?php echo $m['medecin_id']; ?>" class="btn-xs btn-xs-info"><i class="bi bi-eye"></i> Voir</a>
                            <form method="POST" style="display:inline" onsubmit="return confirmAction(this,'<?php echo $m['statut']==='actif'?'suspendre':'activer'; ?>','<?php echo htmlspecialchars($m['prenom'].' '.$m['nom']); ?>')">
                                <input type="hidden" name="action" value="<?php echo $m['statut']==='actif'?'suspendre':'activer'; ?>">
                                <input type="hidden" name="utilisateur_id" value="<?php echo $m['utilisateur_id']; ?>">
                                <button type="submit" class="btn-xs <?php echo $m['statut']==='actif'?'btn-xs-warning':'btn-xs-success'; ?>">
                                    <i class="bi bi-<?php echo $m['statut']==='actif'?'pause-circle':'play-circle'; ?>"></i>
                                    <?php echo $m['statut']==='actif'?'Suspendre':'Activer'; ?>
                                </button>
                            </form>
                            <form method="POST" style="display:inline" onsubmit="return confirm('Supprimer définitivement ce médecin ?')">
            ...
            <?php else: ?>
            <div class="empty"><i class="bi bi-person-x"></i><p>Aucun médecin trouvé</p></div>
            <?php endif; ?>
        </div>
    </div>
</div>
...
<script>
function confirmAction(form, action, nom) {
    const msgs = { suspendre: `Suspendre Dr. ${nom} ?`, activer: `Activer Dr. ${nom} ?` };
    return confirm(msgs[action] || 'Confirmer ?');
}
</script>
</body>
</html>